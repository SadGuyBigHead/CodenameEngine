...
class Slider extends FlxSprite
{
	...
	override function update(elapsed:Float)
	{
		...
		else
		{
			switch (animation.curAnim.name)
			{
				case 'selected':
					if (!selected)
						playAnimOrDefault('deselect', 'unselected');
				case 'unselected':
					if (selected)
						playAnimOrDefault('selecting', 'selected');
				case 'selecting' | 'segment':
			...
			if (__curSegments != (__curSegments = CoolUtil.minInt(segments, Math.floor(value * (segments + 1)))) && playSound)
			{
				...
				if (animation.curAnim.name == 'selected' && animation.exists('segment'))
					animation.play('segment');
			}
		}
	}
	...
	override function drawComplex(camera:FlxCamera)
	{
		...
		var width = barWidth / (segments + 1),
			x = drawCorner(camera, 0, false);
		var end = barWidth - __cornerWidth,
			nextSegment = width - (segments == 0 ? 0 : __segmentWidth * 0.5);
		while (x < end)
		{
			if (x == (x = drawBar(camera, x, Math.min(nextSegment - x, end - x))))
				break;
		...
		if (showSlider)
		{
			...
			prepareDrawMatrix(camera,
				-_frame.sourceSize.x * 0.5
				+ (__flipX ? barWidth - __valueWidth : __valueWidth),
				-(_frame.sourceSize.y - barHeight) * 0.5);
	...
	function drawCorner(camera:FlxCamera, x:Float, flip:Bool):Float
	{
		...
		if (__valueWidth > x)
		{
			...
			if (flip)
				__barClipRect.set(__cornerWidth - __valueWidth + x, 0, __valueWidth - x, frame.sourceSize.y);
			else
				__barClipRect.set(0, 0, __valueWidth - x, frame.sourceSize.y);
		...
		if (__valueWidth - x < __cornerWidth)
		{
			...
			if (flip)
				__barClipRect.set(0, 0, __cornerWidth - __valueWidth + x, frame.sourceSize.y);
			else
				__barClipRect.set(__valueWidth - x, 0, __cornerWidth - __valueWidth + x, frame.sourceSize.y);
			__barFrame = frame.clipTo(__barClipRect, __barFrame);
			__barFrame.prepareMatrix(_matrix, ANGLE_0, flip != __flipX != camera.flipX, __flipY != camera.flipY);
			prepareDrawMatrix(camera, __flipX ? barWidth - x - frame.sourceSize.x : x, 0);
			forceDrawFrame(camera, __barFrame);
	...
	override function resetHelpers()
	{
		...
		inline function checkSize(type)
		{
			...
			{
				case 0:
					__barWidth = Math.max(__barWidth, width);
				case 1:
					__cornerWidth = Math.max(__cornerWidth, width);
				case 2:
					__segmentWidth = Math.max(__segmentWidth, width);
			}
		}
		...
		for (type in 0...3)
		{
			if ((anim = getBarAnim(type, false)) != null)
				checkSize(type);
			if ((anim = getBarAnim(type, true)) != null)
				checkSize(type);
		}
	...
	function set_segments(v:Int):Int
		return segments = CoolUtil.maxInt(v, 0);
}
