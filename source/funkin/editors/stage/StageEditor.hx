...
class StageEditor extends UIState
{
			...
			},
			{
				...
				childs: [
					...
					null,
					{
						...
						onSelect: (_) ->
						{
							openSubState(new UISoftcodedWindow("layouts/stage/stageInfoScreen", [
		...
		for (i => sprite in order)
		{
			...
			if (xml != null)
			{
				...
				else if (sprite is FunkinSprite)
				{
					...
					var button:StageElementButton = (type == "box" || type == "solid") ? new StageSolidButton(0, 0, sprite,
						xml) : new StageSpriteButton(0, 0, sprite, xml);
	...
	public override function update(elapsed:Float)
	{
		...
		stageCamera.scroll.set(lerp(stageCamera.scroll.x, nextScroll.x, 0.35), lerp(stageCamera.scroll.y, nextScroll.y, 0.35));
	...
	function _file_save(_)
	{
		...
		CoolUtil.safeSaveFile('${Paths.getAssetsRoot()}/data/stages/${__stage}.xml', buildStage());
		undos.save();
		#else
		_file_saveas(_);
		#end
	...
	function savePointToXml(xml:Xml, name:String, point:FlxPoint, ?defaultValueX:Float, ?defaultValueY:Float)
	{
		if (point == null)
			return xml;
		if (defaultValueY == null)
			defaultValueY = defaultValueX;
		if (point.x == point.y)
		{
	...
	{
		if (sprite == null || sprite.offset == null)
			return; // destroyed
		...
		if (DrawUtil.line == null)
			DrawUtil.createDrawers();
		DrawUtil.line.cameras = [gizmosCamera];
		DrawUtil.line.alpha = 0.85;
		DrawUtil.dot.cameras = [gizmosCamera];
		DrawUtil.dot.alpha = 1;
		...
		if (sprite != null)
			sprite.extra.set(exID("buttonBoxes"), buttonBoxes);
	...
	public static function calcSpriteBounds(sprite:FunkinSprite)
	{
		...
		// }
		@:privateAccess corners[corners.length - 1].add(dotCheckSize * 0.35 * sprite._cosAngle
			+ dotCheckSize * 0.35 * sprite._sinAngle,
			-dotCheckSize * 0.35 * sprite._cosAngle
			+ dotCheckSize * 0.35 * sprite._sinAngle);
		...
		for (i => edge in edges)
		{
			if (checkDot(buttonBoxes[i]))
			{
				...
				currentCursor = switch (edge)
				{
					...
					case CENTER_CIRCLE:
						#if (mac) FlxG.mouse.pressed ? DRAG : DRAG_OPEN #elseif (linux) FlxG.mouse.pressed ? DRAG : CLICK #else MOVE #end;
	...
	function checkLine(point1:FlxPoint, point2:FlxPoint, dx:Float, dy:Float)
	{
		...
		return (mousePoint.x >= leftRegion && mousePoint.x <= rightRegion) && (mousePoint.y >= topRegion && mousePoint.y <= bottomRegion);
	}
}
