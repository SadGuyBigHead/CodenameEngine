...
class StageSelectionScreen extends EditorTreeMenuScreen
{
	...
	public function saveStage(creation:StageCreationData)
	{
		...
		{
			parent.openSubState(new UIWarningSubstate(TU.translate("stageCreationScreen.warnings.stage-exists-title"),
				TU.translate("stageCreationScreen.warnings.stage-exists-body"), [
				{
		...
		#if sys
		// Save File
		CoolUtil.safeSaveFile('${Paths.getAssetsRoot()}/data/stages/${creation.name}.xml',
