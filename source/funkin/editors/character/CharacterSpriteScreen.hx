...
class CharacterSpriteScreen extends UISubstateWindow
{
	...
	public override function create()
	{
		...
		imageExplorer = new UIImageExplorer(20, windowSpr.y + 30 + 16 + 20, imagePath, 320, 58, (_, _) ->
		{
			onLoadImage();
		}, "images/characters");
		add(imageExplorer);
		addLabelOn(imageExplorer, "").applyMarkup(translate('charImageFile'), [new FlxTextFormatMarkerPair(new FlxTextFormat(0xFFAD1212), "$")]);
		...
		saveButton = new UIButton(windowSpr.x + windowSpr.bWidth - 20, windowSpr.y + windowSpr.bHeight - 20, TU.translate("editor.saveClose"), function()
		{
			...
			{
				onSave('${imageExplorer.saveData.directory.length > 0 ? '${imageExplorer.saveData.directory}/' : ""}' + imageExplorer.imageName,
					imageExplorer.isAtlas);
				close();
			});
		}, 125);
	...
	public function onLoadImage()
	{
		...
		if (imageExplorer == null || imageExplorer.imageFiles == null)
			return;
	...
	public inline function addToUndo()
	{
		...
		CoolUtil.safeSaveFile('./.temp/__undo__${Type.getClassName(Type.getClass(FlxG.state))}__${idCounter}.cneisd',
			UIImageExplorer.serializeSaveDataGlobal(ogImageSaveData));
		CoolUtil.safeSaveFile('./.temp/__redo__${Type.getClassName(Type.getClass(FlxG.state))}__${idCounter}.cneisd',
			UIImageExplorer.serializeSaveDataGlobal(imageExplorer.getSaveData()));
	}
}
