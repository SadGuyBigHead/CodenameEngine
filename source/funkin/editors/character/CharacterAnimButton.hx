...
class CharacterAnimButton extends UIButton
{
	...
	public function new(x:Float, y:Float, animData:AnimData, parent:CharacterAnimsWindow)
	{
		...
		nameTextBox.onChange = (newName:String) ->
		{
		...
		animTextBox.onChange = (newAnim:String) ->
		{
		...
		loopedCheckbox.onChecked = (newLooping:Bool) ->
		{
			this.changeLooping(newLooping);
		};
		...
		deleteButton = new UIButton(0, 0, null, () ->
		{
			parent.deleteAnimation(this);
		}, 28 * 2, 24);
	...
	public function changeName(newName:String, addToUndo:Bool = true) @:privateAccess {
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CAnimEditName(ID, oldName, newName));
	}

	public function changeAnim(newAnim:String, addToUndo:Bool = true) @:privateAccess {
		var animData:AnimData = parent.character.animDatas[anim];
		if (newAnim == animData.anim)
			return;
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CAnimEditAnim(ID, oldAnim, newAnim));
	}
	...
	public function changeOffset(newOffsetX:Null<Float>, newOffsetY:Null<Float>, addToUndo:Bool = true)
	{
		...
		if (newOffsetX != null)
			animData.x = newOffsetX;
		if (newOffsetY != null)
			animData.y = newOffsetY;
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CAnimEditOffset(ID, oldPosition, FlxPoint.get(animData.x, animData.y)));
	}
	...
	public function changeFPS(newFPS:Float, addToUndo:Bool = true) @:privateAccess {
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CAnimEditFPS(ID, oldFPS, newFPS));
	}
	...
	public function changeLooping(newLooping:Bool, addToUndo:Bool = true) @:privateAccess {
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CAnimEditLooping(ID, newLooping));
	}
	...
	public function changeLabel(newLabel:Bool, addToUndo:Bool = true) @:privateAccess {
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CAnimEditLabel(ID, newLabel));
	}
	...
	public override function draw()
	{
		...
		if (!closed && parent.displayAnimsFramesList.exists(anim))
		{
			...
			parent.displayWindowSprite.follow(this, 16
				+ (128 / 2)
				- ((parent.displayWindowSprite.frame.sourceSize.x * displayData.scale) / 2),
				8
				+ 32
				+ 8
				+ 2
				+ 11
				+ (128 / 2)
				- ((parent.displayWindowSprite.frame.sourceSize.y * displayData.scale) / 2));
	...
	{
		field.applyMarkup('${anim} '
			+ (valid ? '(${parent.character.animOffsets[anim].x}, ${parent.character.animOffsets[anim].y})' : "|(Invalid Animation!!!)|"),
