...
class CharacterEditor extends UIState
{
	...
	public override function update(elapsed:Float)
	{
		...
		charCamera.scroll.set(lerp(charCamera.scroll.x, _nextScroll.x, 0.35), lerp(charCamera.scroll.y, _nextScroll.y, 0.35));
	...
	function _file_save(_)
	{
		...
		CoolUtil.safeSaveFile('${Paths.getAssetsRoot()}/data/characters/${character.curCharacter}.xml', buildCharacter());
		#else
		_file_saveas(_);
		#end
		undos.save();
	...
	function clearOffsets(addToUndo:Bool = true)
	{
		...
		if (addToUndo)
			undos.addToUndo(CCharClearOffsets(oldOffsets));
	}

	function _change_offset(x:Float, y:Float)
	{
		if (characterFakeAnim == null)
			return;
		_set_offset(character.animOffsets[characterFakeAnim].x - x, character.animOffsets[characterFakeAnim].y - y);
	}

	function _set_offset(x:Float, y:Float)
	{
		if (characterAnimsWindow.animButtons[characterFakeAnim] == null || !characterAnimsWindow.animButtons[characterFakeAnim].valid)
			return;
		characterAnimsWindow.animButtons.get(characterFakeAnim).changeOffset(FlxMath.roundDecimal(x, 2), FlxMath.roundDecimal(y, 2));
	}
	...
	function buildStagesUI()
	{
		...
		if (stageTopButton != null)
			stageTopButton.contextMenu = newChilds;
		return newChilds;
	}
	...
	function updateStagePositions(stagePositions:Array<String>) @:privateAccess {
		...
		{
			characterPropertiesWindow.testAsDropDown.items = UIDropDown.getItems(characterPropertiesWindow.testAsDropDown.options = stagePositions);
		}
		else
			characterPropertiesWindow.testAsDropDown.items = UIDropDown.getItems(characterPropertiesWindow.testAsDropDown.options = ["NONE"]);
	}
	...
	public function changeCharacterDesginedAs(player:Bool, addToUndo:Bool = true) @:privateAccess {
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CCharEditDesignedAs(player));
	}
	...
	function _animation_up(_)
		playAnimation(characterAnimsWindow.animsList[
			FlxMath.wrap(characterAnimsWindow.animsList.indexOf(characterFakeAnim) - 1, 0, characterAnimsWindow.animsList.length - 1)
		]);
	...
	public function playAnimation(anim:String) @:privateAccess {
		...
		else
		{
			...
			if (validAnimation != null)
				character.playAnim(validAnimation, true);
