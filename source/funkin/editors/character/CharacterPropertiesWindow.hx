...
class CharacterPropertiesWindow extends UISliceSprite
{
	...
	public function new(x:Float, y:Float, character:Character) @:privateAccess {
		...
		flipXCheckbox = new UICheckbox(scaleStepper.x + 22, scaleStepper.y + 32 + 14, translate("charFlipped"),
			character.isPlayer ? !character.__baseFlipped : character.__baseFlipped);
		flipXCheckbox.onChecked = (checked:Bool) ->
		{
			this.changeFlipX(checked);
		};
		...
		antialiasingCheckbox.onChecked = (checked:Bool) ->
		{
			this.changeAntialiasing(checked);
		};
	...
	public function changePosition(newPosX:Null<Float>, newPosY:Null<Float>, addToUndo:Bool = true)
	{
		...
		{
			if (newPosX != null && newPosX == character.globalOffset.x)
				return;
			if (newPosY != null && newPosY == character.globalOffset.y)
				return;
		}
		var oldPosition:FlxPoint = character.globalOffset.clone();

		if (newPosX != null)
			character.globalOffset.x = newPosX;
		if (newPosY != null)
			character.globalOffset.y = newPosY;
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CCharEditPosition(oldPosition, character.globalOffset.clone()));
		else
			oldPosition.put();
	}
	...
	public function changeScale(newScale:Float, addToUndo:Bool = true)
	{
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CCharEditScale(oldScale, newScale));
	}
	...
	public function editCharacterInfo(info:CharacterExtraInfo, addToUndo:Bool = true)
	{
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CCharEditInfo(oldInfo, info));
	}
	...
	public function changeFlipX(newFlipX:Bool, addToUndo:Bool = true) @:privateAccess {
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CCharEditFlipped(newFlipX));
	}
	...
	public function changeCamPosition(newPosX:Null<Float>, newPosY:Null<Float>, addToUndo:Bool = true)
	{
		...
		{
			if (newPosX != null && newPosX == character.cameraOffset.x)
				return;
			if (newPosY != null && newPosY == character.cameraOffset.y)
				return;
		}
		var oldCamPosition:FlxPoint = character.cameraOffset.clone();

		if (newPosX != null)
			character.cameraOffset.x = newPosX;
		if (newPosY != null)
			character.cameraOffset.y = newPosY;
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CCharEditCamPosition(oldCamPosition, character.cameraOffset.clone()));
		else
			oldCamPosition.put();
	}
	...
	{
		if (character.antialiasing == newAntialiasing)
			return;
		...
		if (addToUndo)
			CharacterEditor.undos.addToUndo(CCharEditAntialiasing(newAntialiasing));
	}
