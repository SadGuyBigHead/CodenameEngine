...
class CharterSelectionScreen extends EditorTreeMenuScreen
{
	...
	public function openSongOption(s:ChartMetaData, first = true)
	{
		...
		for (d in s.difficulties)
			if (d != '')
				screen.add(makeChartOption(d, isVariant ? s.variant : null, s.name));
		if (s.difficulties.length > 0 && s.variants.length > 0)
			screen.add(new Separator()); // Create a separator only when there are both difficulty and variant options available.
		for (v in s.variants)
			if (s.metas.get(v) != null)
				screen.add(makeVariationOption(s.metas.get(v)));
		...
		if (!first)
			screen.curSelected = (s.difficulties.length + s.variants.length) > 0 ? 1 : 0;
		else
		{
	...
	public function saveSong(creation:SongCreationData, ?callback:String->Void)
	{
		...
		{
			parent.openSubState(new UIWarningSubstate(TU.translate("chartCreation.warnings.song-exists-title"),
				TU.translate("chartCreation.warnings.song-exists-body"), [
				{
		...
		{
			if (curSong.variants == null)
				curSong.variants = [];
			if (!curSong.variants.contains(variant))
	...
	public function saveChart(name:String, data:ChartData)
	{
		...
		{
			parent.openSubState(new UIWarningSubstate(TU.translate("chartCreation.warnings.chart-exists-title"),
				TU.translate("chartCreation.warnings.chart-exists-body"), [
				{
					label: TU.translate("editor.ok"),
					color: 0xFFFF0000,
					onClick: (t) -> {}
		...
		var idx = 0;
		while (!(screen.members[idx] is Separator || idx >= screen.members.length))
			idx++;
