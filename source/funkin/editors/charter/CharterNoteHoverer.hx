...
class CharterNoteHoverer extends CharterNote
{
	...
	public function new()
	{
		...
		visible = sustainSpr.visible = false;
	...
	public override function update(elapsed:Float) @:privateAccess {
		...
		switch (Charter.instance.gridActionType)
		{
			case NONE:
				...
				{
					step = CoolUtil.bound(FlxG.keys.pressed.SHIFT ? ((__mousePos.y - 20) / 40) : Charter.instance.quantStep(__mousePos.y / 40), 0,
						Charter.instance.__endStep - 1);
					id = Math.floor(__mousePos.x / 40);
					y = step * 40;
					x = id * 40;
					visible = true;
					sustainSpr.visible = typeVisible = false;
					...
					visible = false;
			case NOTE_DRAG:
				visible = sustainSpr.visible = typeVisible = true;
				__doAnim = false;
			default:
				visible = sustainSpr.visible = typeVisible = false;
				__doAnim = false;
		}
	}
				...
				if (Charter.instance.gridActionType == NOTE_DRAG)
				{
					...
					for (s in Charter.selection)
					{
						if (s != null && s.draggable && s is CharterNote)
						{
							...
							x = (id = newID) * 40;
							y = CoolUtil.bound(y, 0, (Charter.instance.__endStep * 40) - height);
