...
class CharterEventScreenNew extends MusicBeatSubstate
{
	...
	public override function create()
	{
		...
		eventsList.addButton.callback = () ->
		{
			...
			openSubState(new CharterEventTypeSelection(function(eventName)
			{
				...
				eventsList.add(new EventButtonNew(events[events.length - 1], CharterEvent.generateEventIcon(events[events.length - 1]), events.length - 1,
					this, eventsList));
		...
		eventsList.cameraSpacing = 0;
		eventsList.topHeight = 0;
	...
	public function boundWindow()
	{
		...
		screenPos.x -= chartEvent.global ? -8 : 68 + 12;
		screenPos.y += 8;
		...
		if (screenSpaceY > -8)
		{ // border of 8 of screen >:D
			bg.bHeight -= cast screenSpaceY + 8;
			cam.height -= cast screenSpaceY + 8;

			screenSpaceY -= 20; // events list is slight diff with its camera
			if (screenSpaceY > -8)
				eventsList.bHeight -= cast screenSpaceY + 8;
		}

		cam.x = screenPos.x;
		cam.y = screenPos.y;

		eventCam.x = cam.x + 10 + 75 + 10;
		eventCam.y = cam.y + 10;
		eventCam.width = bg.bWidth - (10 + 75 + 10);
		eventCam.height = bg.bHeight - 20;
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.justPressed && (FlxMath.inBounds(mousepoint.x, 0, bg.bWidth) && FlxMath.inBounds(mousepoint.y, 0, bg.bHeight)))
			__clickedWhileHovering = true;
		...
		if (lastParamSprite != null)
			boundingParamY = lastParamSprite.y;
		...
		if (FlxMath.inBounds(mousepoint.x, 0, bg.bWidth) && FlxMath.inBounds(mousepoint.y, 0, bg.bHeight))
		{
			...
			if (FlxMath.inBounds(mousepoint.x, 10 + 75 + 10, eventCam.width) && FlxMath.inBounds(mousepoint.y, 10, eventCam.height))
			{
				...
				if (lastParamSprite != null)
				{
					if (lastParamSprite is UISliceSprite)
						boundingParamYHeight += cast(lastParamSprite, UISliceSprite).bHeight;
					else if (lastParamSprite is FlxSprite)
						boundingParamYHeight += cast(lastParamSprite, FlxSprite).height;
				}
		...
		else if (Charter.instance.curContextMenu == null
			&& ((FlxG.mouse.justReleased && !__clickedWhileHovering) || FlxG.mouse.wheel != 0))
		{
		...
		if (FlxG.mouse.justReleased)
			__clickedWhileHovering = false;

		sinner += elapsed;

		downIndicator.setPosition(((eventCam.width / 2))
			- (downIndicator.fieldWidth / 2),
			(eventCam.height - downIndicator.height)
			- 2
			- (FlxMath.fastSin(sinner * 2) * 2));
		...
		events[curEvent].params = [
			for (p in paramsFields)
			{
				if (p is UIDropDown)
				{
					...
					if (dataParams[paramsFields.indexOf(p)].type == TStrumLine)
						cast(p, UIDropDown).index;
					else
						cast(p, UIDropDown).label.text;
				}
				...
				else if (p is UITextBox) cast(p, UITextBox).label.text; else if (p is UICheckbox) cast(p, UICheckbox).checked; else if (p is UIColorwheel)
					cast(p, UIColorwheel).curColor; else null;
			}
		];
	}
