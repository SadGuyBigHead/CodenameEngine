...
class ChartCreationScreen extends UISubstateWindow
{
	...
	public override function create()
	{
		...
		strumLineList = new UIButtonList<StrumLineButton>(difficultyNameTextBox.x, difficultyNameTextBox.y + difficultyNameTextBox.bHeight + 36, 620,
			(552 - 179) - 16, null, FlxPoint.get(620, 246), null, 6);
		...
		add(strumLineList);
		addLabelOn(strumLineList,
			"").applyMarkup(TU.translate("chartCreation.strumLineList"), [new FlxTextFormatMarkerPair(new FlxTextFormat(0xFFAD1212), "$")]);

		var strumlineFormat = TU.getRaw("chartCreation.strumLine.format");
		strumLineList.dragCallback = (object:StrumLineButton, oldIndex:Int, newIndex:Int) ->
		{
			object.idText.text = strumlineFormat.format([newIndex]);
		};
	...
	public function createChart()
	{
		...
		if (onSave != null)
			onSave(difficultyNameTextBox.label.text, chartData);
	}
}
...
class StrumLineButton extends UIButton
{
	...
	public function new(id:Int, strumLine:ChartStrumLine, parent:UIButtonList<StrumLineButton>)
	{
		...
		charactersList.addButton.callback = function() charactersList.add(new CompactCharacterButton(TU.translate("charterStrumLine.newChar"),
			subState.charFileList, charactersList));
		...
		var startingPos:FlxPoint = strumLine.strumLinePos == null ? FlxPoint.get(strOffset, 50) : FlxPoint.get(strOffset, strumLine.strumPos[1]);
		...
		visibleCheckbox = new UICheckbox(hudYStepper.x + hudYStepper.bWidth + 42, hudYStepper.y + 9, TU.translate("charterStrumLine.visible"),
			strumLine.visible == null ? true : strumLine.visible);
		...
		usesChartScrollSpeed = new UICheckbox(scrollSpeedStepper.x + 104, typeDropdown.y + 135, TU.translate("charterStrumLine.useChartScrollSpeed"),
		...
		deleteIcon = new FlxSprite(deleteButton.x + ((deleteButton.bWidth / 2) - (15 / 2)),
			deleteButton.y + ((deleteButton.bHeight / 2) - (16 / 2))).loadGraphic(Paths.image('editors/delete-button'));
	...
	public override function update(elapsed:Float)
	{
		...
		cameraClipShader.hset("clipRect", [
			0,
			(-y - (8 + 26)) + cameras[0].scroll.y,
			strumLineList.bWidth,
			strumLineList.bHeight
		]);
	}
}
