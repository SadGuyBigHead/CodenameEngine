...
class CharterEventTypeSelection extends UISubstateWindow
{
	...
	public override function create()
	{
		...
		for (change in Conductor.bpmChangeMap)
		{
			if (change.continuous
				...
				&& MathUtil.lessThan(eventStepTime, change.endStepTime))
			{
		...
		for (k => eventName in EventsData.eventsList)
		{
			...
			if (disableConductorEvents
				&& (eventName == "Time Signature Change" || eventName == "Continuous BPM Change" || eventName == "BPM Change")
				|| (disableOnlyContinuousChanges && eventName == "Continuous BPM Change"))
			{
	...
	override function update(elapsed:Float)
	{
		...
		downIndicator.setPosition((buttonsBG.bWidth / 2) - (downIndicator.fieldWidth / 2),
			(buttonsBG.bHeight - downIndicator.height - 22) - (FlxMath.fastSin(sinner * 2) * 4));

		upIndicator.alpha = CoolUtil.fpsLerp(upIndicator.alpha, (buttons[0].y + buttonCameras.scroll.y > buttons[0].bHeight) ? 1 : 0, 1 / 3);
		downIndicator.alpha = CoolUtil.fpsLerp(downIndicator.alpha, (buttonCameras.scroll.y + buttonCameras.height > buttons[buttons.length - 1].y) ? 0 : 1,
			1 / 3);
	}
