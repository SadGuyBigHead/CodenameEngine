...
class CharterStrumlineScreen extends UISubstateWindow
{
	...
	public override function create()
	{
		...
		var title:UIText;
		add(title = new UIText(windowSpr.x + 20, windowSpr.y + 30 + 16, 0,
			TU.translate("charterStrumlineScreen.title" + (creatingStrumLine ? "-creating" : "-editing")), 28));

		var charFileList = Character.getList(true);
		if (charFileList.length == 0)
			charFileList = Character.getList(false);

		charactersList = new UIButtonList<CharacterButton>(15, title.y + title.height + 36, 250, 269, null, FlxPoint.get(250, 54), null, 0);
		charactersList.addButton.callback = () -> charactersList.add(new CharacterButton(0, 0, TU.translate("charterStrumLine.newChar"), charFileList,
			charactersList));
		...
		typeDropdown = new UIDropDown(charactersList.x + charactersList.bWidth + 16, charactersList.y, 200, 32, ["OPPONENT", "PLAYER", "ADDITIONAL"],
			strumLine.type);
		add(typeDropdown);
		addLabelOn(typeDropdown, TU.translate("charterStrumLine.type"));

		usesChartScrollSpeed = new UICheckbox(typeDropdown.x + 104, typeDropdown.y + 135, TU.translate("charterStrumLine.useChartScrollSpeed"),
			strumLine.scrollSpeed == null);
		...
		scrollSpeedStepper = new UINumericStepper(typeDropdown.x, typeDropdown.y + 128,
			usesChartScrollSpeed.checked ? PlayState.SONG.scrollSpeed : strumLine.scrollSpeed, 0.1, 2, 0, 10, 82);
		...
		hudScaleStepper = new UINumericStepper(typeDropdown.x, typeDropdown.y + 64, strumLine.strumScale == null ? 1 : strumLine.strumScale, 0.001, 2, null,
			null, 84);
		add(hudScaleStepper);
		addLabelOn(hudScaleStepper, TU.translate("charterStrumLine.scale"));

		hudSpacingStepper = new UINumericStepper(hudScaleStepper.x + 80 - 32 + 26, hudScaleStepper.y,
			strumLine.strumSpacing != null ? strumLine.strumSpacing : 1, 0.001, 10, null, null, 84);
		...
		var startingPos:FlxPoint = strumLine.strumPos == null ? FlxPoint.get(strOffset, 50) : FlxPoint.get(strOffset, strumLine.strumPos[1]);
		...
		visibleCheckbox = new UICheckbox(hudYStepper.x + hudYStepper.bWidth + 42, hudYStepper.y + 9, TU.translate("charterStrumLine.visible"),
			strumLine.visible);
		add(visibleCheckbox);
		...
		closeButton = new UIButton(saveButton.x - 20 - saveButton.bWidth, saveButton.y, TU.translate("editor." + (creatingStrumLine ? "cancel" : "close")),
			function()
			{
		...
		vocalsSuffixDropDown = new UIDropDown(typeDropdown.x, hudScaleStepper.y + 128, 200, 32,
			suffixList, strumLine.vocalsSuffix != null && strumLine.vocalsSuffix != "" && Assets.exists(Paths.voices(Charter.__song.toLowerCase(), "",
				strumLine.vocalsSuffix)) ? suffixList.indexOf(strumLine.vocalsSuffix) : 0);
		add(vocalsSuffixDropDown);
		addLabelOn(vocalsSuffixDropDown, TU.translate("charterStrumLine.vocalSuffix"));

		keyCountStepper = new UINumericStepper(stagePositionDropdown.x, vocalsSuffixDropDown.y, strumLine.keyCount != null ? strumLine.keyCount : 4, 1, 0, 1,
			1000, 84);
		add(keyCountStepper);
	...
	function saveStrumline()
	{
		...
		if (!usesChartScrollSpeed.checked)
			newStrumLine.scrollSpeed = scrollSpeedStepper.value;
		if (onSave != null)
			onSave(newStrumLine);
	}
	...
	override public function update(elapsed:Float)
	{
		...
		var xOffset:Float = StrumLine.calculateStartingXPosFromInitialWidth(hudXStepper.value, hudScaleStepper.value, hudSpacingStepper.value,
			Std.int(keyCountStepper.value));
