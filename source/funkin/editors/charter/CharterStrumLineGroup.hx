...
class CharterStrumLineGroup extends FlxTypedGroup<CharterStrumline>
{
	...
	{
		if (__totalKeyCount != -1)
			return __totalKeyCount;
	...
	public override function update(elapsed:Float)
	{
		...
		for (strumLine in members)
		{
			...
			if (draggable
				&& UIState.state.isOverlapping(strumLine.draggingSprite, @:privateAccess strumLine.draggingSprite.__rect)
		...
		for (i => strum in members)
			if (strum != null && !strum.dragging)
				strum.x = CoolUtil.fpsLerp(strum.x, 40 * strum.startingID, 0.225);
	...
	public function finishDrag(?addToUndo:Bool = true)
	{
		...
		if (addToUndo)
		{
			...
			if (newID != oldID)
				Charter.undos.addToUndo(COrderStrumLine(newID, oldID, newID));
		}
