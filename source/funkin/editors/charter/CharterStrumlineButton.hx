...
class CharterStrumlineButton extends UISprite
{
	...
	public override function update(elapsed:Float)
	{
		...
		if (shakeTimer > 0)
			...
			{
				spr.x += FlxG.random.float(0, 3) * FlxG.random.sign();
				spr.y += FlxG.random.float(0, 3) * FlxG.random.sign();
			}
		...
		button.scale.set(FlxMath.lerp(button.scale.x, buttonScale.x, 1 / 6), FlxMath.lerp(button.scale.y, buttonScale.y, 1 / 6));
