...
class CharterDeleteAnim extends CharterNote
{
	...
	public function new()
	{
		...
		garbageCircle.color = 0xFF880000;
		garbageCircle.alpha = .6;
	...
	public override function update(elapsed:Float)
	{
		for (deleteData in deleteNotes)
		{
			...
			if (deleteData.time < 0)
				deleteNotes.remove(deleteData);
		}
		...
		{
			garbageIcon.setPosition(__mousePos.x
				+ garbageIcon.width / 2
				+ (.65 * FlxG.random.float(-1, 1))
				+ 2,
				__mousePos.y
				- garbageIcon.height
				+ (.65 * FlxG.random.float(-1, 1))
			...
			garbageCircle.setPosition(__mousePos.x - garbageCircle.width / 2, (__mousePos.y - garbageCircle.height / 2) + 2);
		}
	...
	public override function draw() @:privateAccess {
		...
		{
			y = deleteData.note.y
				+ (deleteData.time > deleteTime * .5 ? (deleteData.time / deleteTime) * FlxG.random.float(-1.1, 1.1) : 0); // lunar when no shake :((
			x = deleteData.note.x
				+ (deleteData.time > deleteTime * .5 ? (deleteData.time / deleteTime) * FlxG.random.float(-1.1, 1.1) : 0); // lunar when no shake :((
			...
			sustainSpr.exists = deleteData.note.susLength != 0;
			sustainSpr.alpha = .8;
		...
		if (garbageIcon.alpha > 0)
			garbageIcon.draw();
		if (garbageCircle.alpha > 0)
			garbageCircle.draw();
	}
