...
class CharterAutoSaveUI extends UISliceSprite
{
	...
	public function new(x:Float, y:Float)
	{
		...
		progressBarBack.antialiasing = false;
		...
		progressBar.clipRect = new FlxRect(0, 0, progressBar.width, progressBar.height);
		members.push(progressBar);

		cancelButton = new UIButton(x - (10 + 16), y + 8, null, () ->
		{
			if (cancelled || !showedAnimation)
				return;
			cancelled = true;
			__timer.cancel();
			__tween.cancel();
			...
			for (member in [this, autosavingText])
				member.color = 0xFFE67F7F;
			for (member in [progressBar, progressBarBack])
				member.color = 0xFFE67F7F;
		}, 24, 14);
		cancelButton.frames = Paths.getFrames("editors/ui/grayscale-button");
		cancelButton.color = 0xFFAC3D3D;
	...
	public function startAutoSave(time:Float, successText:String)
	{
		...
		{
			autosavingText.text = successText;
			cancelButton.visible = false;

			icon.animation.curAnim.curFrame = 1;
			for (member in [this, progressBar, progressBarBack, autosavingText])
				member.color = 0xFFA3EC95;
	...
	public function appearAnimation()
	{
		progress = 0;
		icon.animation.curAnim.curFrame = 0;
		for (member in [this, autosavingText, progressBar, progressBarBack])
	...
	public function disappearAnimation(?canned:Bool = false)
	{
		...
		FlxTween.cancelTweensOf(this);
		if (canned)
			FlxTween.tween(this, {x: -320}, .4, {ease: FlxEase.circInOut});
