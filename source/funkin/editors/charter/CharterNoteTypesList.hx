...
class CharterNoteTypesList extends UISubstateWindow
{
	...
	public override function create()
	{
		...
		if (noteTypesFileList.length == 0)
			noteTypesFileList = getNoteTypesList(false);

		noteTypesList = new UIButtonList<NoteTypeButton>(20, title.y + title.height + 10, winWidth - 40, 342 - 85 - 16, null,
			FlxPoint.get(winWidth - 40, (342 - 85 - 16) / 4), null, 0);
		noteTypesList.addButton.callback = () -> noteTypesList.add(new NoteTypeButton(0, 0,
			TU.translate("charterNoteTypesList.editType", [noteTypesList.buttons.members.length]), noteTypesList.buttons.members.length, noteTypesFileList,
			noteTypesList));
		noteTypesList.cameraSpacing = 0;
		noteTypesList.dragCallback = (object:NoteTypeButton, oldIndex:Int, newIndex:Int) ->
		{
			object.IDText.text = '${newIndex}.';
		};
	...
	public function getNoteTypesList(?mods:Bool = false)
	{
		...
		for (path in Paths.getFolderContent(pathString, true, mods ? MODS : BOTH))
...
class NoteTypeButton extends UIButton
{
	...
	{
		if (noteSpr == null)
			members.push(noteSpr = new FlxSprite());
		...
		noteSpr.antialiasing = true;
	}

	override function update(elapsed:Float)
	{
		IDText.x = x + 12;
		IDText.y = (y + bHeight / 2) - (IDText.height / 2);
		noteSpr.x = IDText.x + IDText.width + 2;
		noteSpr.y = (y + bHeight / 2) - (noteSpr.height / 2);
		textBox.x = noteSpr.x + noteSpr.width + 12;
		textBox.y = (y + bHeight / 2) - (textBox.bHeight / 2);
		deleteButton.x = textBox.x + textBox.bWidth + 14;
		deleteButton.y = textBox.y;
