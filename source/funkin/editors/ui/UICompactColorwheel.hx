	...
	public override function update(elapsed:Float)
	{
		...
		if (hovered && FlxG.mouse.justPressed)
		{
			for (sprite in [colorSlider])
			{
				...
				if (FlxMath.inBounds(mousePos.x, spritePos.x - (hitBoxExtenstion / 2), spritePos.x - (hitBoxExtenstion / 2) + (sprite.width + hitBoxExtenstion))
					&& FlxMath.inBounds(mousePos.y, spritePos.y
						- (hitBoxExtenstion / 2),
						spritePos.y
						- (hitBoxExtenstion / 2)
						+ (sprite.height + hitBoxExtenstion)))
				{
