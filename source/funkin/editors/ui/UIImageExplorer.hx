...
class UIImageExplorer extends UIFileExplorer
{
	...
	public function new(x:Float, y:Float, image:String, ?w:Int, ?h:Int, ?onFile:(String, Bytes) -> Void, ?directory:String = "images")
	{
		...
		directoryButton = new UIButton(x + bWidth - (bHeight - 16) - 8, y + 8, null, () ->
		{
			directoryBG.visible = !directoryBG.visible;
		}, 26, 26);
		...
		directoryBG.alpha = 0.9;
		directoryBG.members.push(directoryTextBoxLabel = new UIText(8, 6, 0, TU.getRaw('uiImageExplorer.directory').format([directory]), 12));
		...
		allowDirectories = CoolUtil.isMapEmpty(imageFiles);
		__firstLoad = false;
	...
	public function uploadImage(filePath:String, file:Bytes)
	{
		...
		bWidth = Std.int(uiElement.width) + 32;
		bHeight = Std.int(uiElement.height) + 32 + deleteButton.bHeight + 4;
		uiElement.x = x + 16;
		uiElement.y = y + 16 + deleteButton.bHeight + 4;
		...
		if (animationList.length > 0)
		{
			...
			message.add(', ${TU.getRaw("uiImageExplorer.notFound").format([typeOfFrame])}');
		...
		if (directoryButton.visible)
			fileText.fieldWidth -= directoryButton.bWidth + 12;

		members.push(directoryBG);
		__firstLoad = false;
	}
	...
	public static function saveFilesGlobal(imageData:ImageSaveData, directory:String, ?onFinishSaving:Void->Void, ?checkExisting:Bool = true)
	{
		...
		if (imageData.isAtlas)
			directory += '/${imageData.imageName}';
		...
		if (alreadlyExistingFiles.length > 0 && checkExisting)
			@:privateAccess {
			var buttons = [
				{
			...
			(FlxG.state.subState != null
				&& !FlxG.state._requestSubStateReset ? FlxG.state.subState : FlxG.state).openSubState(new UIWarningSubstate("Alreadly Existing Files!!!",
