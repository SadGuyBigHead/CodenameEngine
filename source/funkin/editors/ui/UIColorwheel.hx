...
class UIColorwheel extends UISliceSprite
{
	...
	public function new(x:Float, y:Float, ?color:Int)
	{
		...
		for (pixelY in 0...Std.int(colorSlider.height))
		{
			...
			for (pixelX in 0...Std.int(colorSlider.width))
				if (colorSlider.pixels.getPixel32(pixelX, pixelY) != FlxColor.TRANSPARENT)
					colorSlider.pixels.setPixel32(pixelX, pixelY, color);
		}
		...
		for (i in 0...3)
		{
			var numStepper:UINumericStepper = new UINumericStepper(colorSlider.x + 18 + 12.5 + (i * 44), colorHexTextBox.y + 28 + 6 + 13 + 6 + .5, 0, 1, 0, 0,
				255, 40 + 28, 28);
			numStepper.antialiasing = true;
			numStepper.ID = i;
			...
			members.push(numStepper);
			rgbNumSteppers.push(numStepper);
		}
	...
	inline function updateColorPickerSelector()
		colorPickerSelector.selector.setPosition(colorPicker.x
			+ (colorPicker.width * saturation)
			- 8,
			colorPicker.y
			+ (colorPicker.height + (colorPicker.height * -brightness))
			- 8);

	inline function updateColorPickerMouse(mousePos:FlxPoint)
	{
		saturation = mousePos.x / colorPicker.width;
		brightness = 1 + -(mousePos.y / colorPicker.height);
	}
	...
	public function updateWheel(checkChanged:Bool = true)
	{
		...
		colorPickerSelector.curColor = color = curColor = FlxColor.fromHSB(hue, saturation, brightness);
		colorSliderSelector.curColor = FlxColor.fromHSB(hue, 1, 1);

		updateColorPickerSelector();
	...
	public override function update(elapsed:Float)
	{
		...
		if (hovered && FlxG.mouse.justPressed)
		{
			for (sprite in [colorPicker, colorSlider])
			{
				...
				if (FlxMath.inBounds(mousePos.x, spritePos.x - (hitBoxExtension / 2), spritePos.x - (hitBoxExtension / 2) + (sprite.width + hitBoxExtension))
					&& FlxMath.inBounds(mousePos.y, spritePos.y - (hitBoxExtension / 2),
						spritePos.y - (hitBoxExtension / 2) + (sprite.height + hitBoxExtension)))
				{
		...
		if (selectedSprite != null)
		{
			...
			if (selectedSprite == colorSlider)
				updateColorSliderMouse(mousePos);
			if (selectedSprite == colorPicker)
				updateColorPickerMouse(mousePos);
			updateWheel();
			spritePos.put();
