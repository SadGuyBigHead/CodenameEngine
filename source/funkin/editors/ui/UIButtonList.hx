...
class UIButtonList<T:UIButton> extends UIWindow
{
	...
	public override function update(elapsed:Float)
	{
		...
		nextscrollY = CoolUtil.bound(nextscrollY - (hovered ? FlxG.mouse.wheel : 0) * 12, -buttonSpacing,
			Math.max((addButton.y + 32 + (buttonSpacing * 1.5)) - buttonCameras.height, -buttonSpacing));
		...
		for (i => button in buttons.members)
		{
			...
			if (button.ID != i)
			{
				if (dragCallback != null)
					dragCallback(cast button, button.ID, i);
