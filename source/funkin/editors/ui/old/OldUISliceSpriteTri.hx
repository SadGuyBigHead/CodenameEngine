...
class OldUISliceSpriteTri extends UISprite
{
	...
	private function __genMesh()
	{
		...
		if (drawTop)
		{
			...
			sliceTL = __genSliceQuad(0, 0, topLeftWidth, topLeftHeight, topleft, null, null);
			...
			if (bWidth > topleft.frame.width + topright.frame.width)
			{
				...
				sliceTM = __genSliceQuad(topLeftWidth, 0, topWidth, topHeight, top, sliceTL, null);
			}
			...
			sliceTR = __genSliceQuad(bWidth - getFixedSize(topright.frame.width, bWidth), 0, topRightWidth, topRightHeight, topright, sliceTM, null);
		}
		...
		if (drawMiddle && bHeight > top.frame.height + bottom.frame.height)
		{
			...
			sliceML = __genSliceQuad(0, top.frame.height, middleLeftWidth, middleHeight, middleleft, null, sliceTL);
			...
			if (bWidth > getFixedSize(middleleft.frame.width, bWidth) + middleright.frame.width)
			{
				...
				sliceMM = __genSliceQuad(topleft.frame.width, top.frame.height, middleWidth, middleHeight, middle, sliceML, sliceTM);
			}

			// MIDDLE RIGHT
			var middleRightWidth:Float = getFixedSize(middleright.frame.width, bWidth);
			sliceMR = __genSliceQuad(bWidth - getFixedSize(middleright.frame.width, bWidth), top.frame.height, middleRightWidth, middleHeight, middleright,
				sliceMM, sliceTR);
		}
		...
		if (drawBottom)
		{
			...
			sliceBL = __genSliceQuad(0, bHeight - getFixedSize(bottomleft.frame.height, bHeight), bottomLeftWidth, bottomLeftHeight, bottomleft, null, sliceML);
			...
			if (bWidth > bottomleft.frame.width + bottomright.frame.width)
			{
				...
				sliceBM = __genSliceQuad(bottomleft.frame.width, bHeight - getFixedSize(bottom.frame.height, bHeight), bottomMiddleWidth, bottomMiddleHeight,
			...
			sliceBR = __genSliceQuad(bWidth - getFixedSize(bottomright.frame.width, bWidth), bHeight - getFixedSize(bottomright.frame.height, bHeight),
				bottomRightWidth, bottomRightHeight, bottomright, sliceBM, sliceMR);
		}
