...
class UISoftcodedWindow extends UISubstateWindow
{
	...
	public override function create()
	{
		...
		if (varExists("winTitle"))
			winTitle = get("winTitle");
		if (varExists("winWidth"))
			winWidth = get("winWidth");
		if (varExists("winHeight"))
			winHeight = get("winHeight");
			...
			for (el in elements)
			{
				...
				var sprite:FlxSprite = switch (type)
				{
					case "image":
						...
						cast add(spr);
					case "solid":
						var spr = new FunkinSprite(x, y).makeSolid(execAtt(el, "width", 100), execAtt(el, "height", 100), execAtt(el, "color", 0xFFFFFFFF));
						cast add(spr);
					case "textbox":
						var text:String = execString(el.getAtt("text"));
						text = Std.string(execAtt(el, "value", text));
						var textBox = new UITextBox(x, y, text, execAtt(el, "width"), execAtt(el, "height"),
							execAtt(el, "multiline", false)); // (el.getAtt("multiline").getDefault("false") == "true"));
						add(textBox);
					...
					case "stepper":
						...
						var stepper = new UINumericStepper(x, y, execAtt(el, "value", 0), execAtt(el, "step", 1), execAtt(el, "precision", 0),
							execAtt(el, "min"), execAtt(el, "max"), execAtt(el, "width"), execAtt(el, "height"));
						add(stepper);
					...
					case "radio":
						...
						radio;
					case "slider":
						var slider = new UISlider(x, y, execAtt(el, "width"), execAtt(el, "value", 0), execAtt(el, "segments", []),
							execAtt(el, "centered", false));
						add(slider);
					...
					case "buttonlist":
						...
						var buttonlist = new UIButtonList<UIButton>(x, y, execAtt(el, "width"), execAtt(el, "height"),
							execString(el.getAtt("title")).getDefault(""), FlxPoint.get(execAtt(el, "buttonSizeX", 0), execAtt(el, "buttonSizeY", 0)),
							FlxPoint.get(execAtt(el, "buttonOffsetX", 0), execAtt(el, "buttonOffsetY", 0)), execAtt(el, "buttonSpacing"));
				...
				if (sprite is UISprite)
					elementMap.set(name, cast sprite);
			}
		}
