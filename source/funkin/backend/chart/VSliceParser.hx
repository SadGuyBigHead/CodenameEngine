...
class VSliceParser
{
			...
			switch (event.e)
			{
				...
				case "PlayAnimation":
					result.events.push({
						...
						params: [
							...
							values.anim,
							values.force == null ? false : values.force
						]
					});
				case "ScrollSpeed":
					...
					result.events.push({
						...
						params: [
							values.ease != "INSTANT",
							values.scroll == null ? 1 : values.scroll,
							values.duration == null ? 4 : values.duration,
							cneEase[0],
							cneEase[1],
				...
				case "SetCameraBop":
					result.events.push({
						...
						params: [
							values.rate == null ? 4 : values.rate,
							values.intensity == null ? 1 : values.intensity
						]
					});
				case "ZoomCamera":
					var cneEase = values.ease == null || values.ease == "INSTANT" ? ["linear", null] : parseEase(values.ease);
					...
					result.events.push({
						...
						params: [
							values.ease != "INSTANT",
							values.zoom == null ? 1 : values.zoom,
							"camGame",
							values.duration == null ? 4 : values.duration,
							cneEase[0],
							cneEase[1],
	...
	public static function encodeMeta(meta:ChartMetaData, ?chart:ChartData):SwagMetadata
	{
		...
		var defChars:SwagCharactersList = addVars.characters != null ? addVars.characters : {
			player: Flags.DEFAULT_CHARACTER,
			girlfriend: Flags.DEFAULT_GIRLFRIEND,
			opponent: Flags.DEFAULT_OPPONENT,
			playerVocals: [],
			opponentVocals: [],
			instrumental: '',
			altInstrumentals: []
		...
		var result:SwagMetadata = {
			...
			playData: {
				stage: defStage,
				characters: defChars,
				songVariations: addVars.songVariations != null ? [
					for (i in 0...addVars.songVariations.length)
					{
						meta.difficulties.remove(addVars.songVariations[i]);
						addVars.songVariations[i];
					}
				] : [],
