...
class EventsData
{
	public static var defaultEventsList:Array<String> = [
		"HScript Call",
		"Camera Movement",
		"Camera Position",
		"Add Camera Zoom",
	...
	public static var defaultEventsParams:Map<String, Array<EventParamInfo>> = [
		...
		"Camera Bop" => [{name: "Amount", type: TFloat(-10, 10, 0.1, 2), defValue: 0.1}],
		"Camera Zoom" => [
		...
		"Continuous BPM Change" => [
			{name: "Target BPM", type: TFloat(1.00, 9999, 0.001, 3), defValue: 100},
			{name: "Time (steps)", type: TFloat(0.25, 9999, 0.25, 2), defValue: 4}
		],
		"Time Signature Change" => [
			{name: "Target Numerator", type: TFloat(1), defValue: 4},
			{name: "Target Denominator", type: TFloat(1), defValue: 4},
			{name: "Denominator is Steps Per Beat", type: TBool, defValue: false}
		],
		"Scroll Speed Change" => [
		...
		for (file in Paths.getFolderContent('data/events/', true, BOTH))
		{
			...
			try
			{
				...
				for (paramData in data.params)
				{
					...
					{
						trace('Error parsing event param ${paramData.name} - ${eventName}: $e');
			...
			{
				trace('Error parsing file $file: $e');
			}
		}
