...
class HashLinkFixer
{
	public static function build():Array<Field>
	{
		...
		if (cl.isAbstract || cl.isExtern || cl.isInterface || cl.params.length > 0 || cl.name.endsWith("_Impl_") || cl.name.endsWith("_HSX")
			|| cl.name.endsWith("_HSC"))
			return fields;
				...
				case FFun(func):
					...
					switch (hlNativeMeta.params)
					{
						...
						case [_.expr => EConst(CFloat(version))]:
							...
							else hlNativeMeta.params = [macro "std", macro $v{f.name}];
						default:
					}
