...
class FunkinSocket implements IFlxDestroyable
{
	...
	public function bind(?expectingConnections:Int = 1):FunkinSocket
	{
		Logs.traceColored([
			...
			Logs.logText('Binding to ', NONE),
			Logs.logText(host.toString(), YELLOW),
			Logs.logText(':', NONE),
			Logs.logText(Std.string(port), CYAN),
		]);
	...
	public function connect():FunkinSocket
	{
		Logs.traceColored([
			Logs.logText('[FunkinSocket] ', BLUE),
			Logs.logText('Connecting to ', NONE),
			Logs.logText(host.toString(), YELLOW),
			Logs.logText(':', NONE),
			Logs.logText(Std.string(port), CYAN),
		]);
	...
	public function close()
	{
		try
		{
			...
			Logs.traceColored([
				Logs.logText('[FunkinSocket] ', BLUE),
				Logs.logText('Closing socket from ', NONE),
				Logs.logText(host.toString(), YELLOW),
				Logs.logText(':', NONE),
				Logs.logText(Std.string(port), CYAN),
			]);
