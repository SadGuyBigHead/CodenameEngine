...
class FunkinWebSocket implements IFlxDestroyable
{
	...
	**/
	public var onMessage:FlxTypedSignal<OneOfThree<String, Bytes, FunkinPacket>->Void> = new FlxTypedSignal<OneOfThree<String, Bytes, FunkinPacket>->
		Void>(); // cursed 😭😭
	...
	**/
	...
	public function get_handshakeHeaders():Map<String, String>
	{
		return this._ws.additionalHeaders;
	}
	...
	**/
	private function _onMessage(message:MessageType):Void
	{
		...
		switch (message)
		{
			...
			default:
				trace('Unknown message type: ${message}');
		}
	...
	**/
	public function open():FunkinWebSocket
	{
		Logs.traceColored([
			...
			Logs.logText('Opening WebSocket to ', NONE),
			Logs.logText(url, YELLOW),
		]);
		try
		{
			this._ws.open();
	...
	**/
	public function send(data:Dynamic):Bool
	{
		try
		{
			...
			{
				if (data is String)
					metrics.updateBytesSent(Bytes.ofString(data).length);
				else if (data is Bytes || data is ByteArrayData)
					metrics.updateBytesSent(data.length);
				else if (data is FunkinPacket)
					metrics.updateBytesSent(data.toBytes().length);
			}
	...
	**/
	public function close():Void
	{
		Logs.traceColored([
			Logs.logText('[FunkinWebSocket] ', CYAN),
			Logs.logText('Closing WebSocket from ', NONE),
			Logs.logText(url, YELLOW),
		]);
		this._ws.close();
	}
