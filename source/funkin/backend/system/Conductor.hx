...
final class Conductor
{
	public static function getBeats(?every:BeatType, interval:Float, offset:Float = 0):Float
	{
		...
		if (interval <= 0)
			...
			return Math.floor((beat - offset) / interval) * interval;
	}
	...
	private static function mapBPMChange(curChange:BPMChangeEvent, time:Float, bpm:Float):BPMChangeEvent
	{
		...
		stepTime = (curChange.continuous ? curChange.endStepTime : curChange.stepTime)
			+ (time - (curChange.continuous ? curChange.endSongTime : curChange.songTime)) / (15000 / curChange.bpm);
	...
	public static function mapBPMChanges(song:ChartData)
	{
		...
		var events:Array<ChartEvent> = [
			for (e in song.events)
				if (e.params != null && validEventNames.contains(e.name)) e
		];
	...
	private static function mapEvent(e:ChartEvent, curChange:BPMChangeEvent)
	{
		...
		{ // ensure that you cannot place any conductor events during a continuous change
		...
		if (n != 0)
		{
			...
			while (i < n)
			{
				...
				while (i2 < i)
				{
					x = 0;
					ce = grpEvents[i2++];
					while (x < ce.events.length)
						if (validEventNames.contains((e = ce.events[x++]).name))
						{
							events[j++] = e;
							if (eventTime != 0)
								e.time = eventTime;
				...
				j = 0;
				while (j < x)
					curChange = mapEvent(events[j++], curChange);
	...
	public static function getTimeInChangeIndex(time:Float, index:Int = 0):Int
	{
		...
		{
			while (--index > 0)
				if (time > bpmChangeMap[index].songTime)
					return index;
			return 0;
		}
		else
		{
			for (i in index...bpmChangeMap.length)
				if (bpmChangeMap[i].songTime > time)
					return i - 1;
			return bpmChangeMap.length - 1;
		}
	}
	...
	public static function getStepsInChangeIndex(stepTime:Float, index:Int = 0):Int
	{
		...
		{
			while (--index > 0)
				if (stepTime > bpmChangeMap[index].stepTime)
					return index;
			return 0;
		}
		else
		{
			for (i in index...bpmChangeMap.length)
				if (bpmChangeMap[i].stepTime > stepTime)
					return i - 1;
			return bpmChangeMap.length - 1;
		}
	}
	...
	public static function getBeatsInChangeIndex(beatTime:Float, index:Int = 0):Int
	{
		...
		{
			while (--index > 0)
				if (beatTime > bpmChangeMap[index].beatTime)
					return index;
			return 0;
		}
		else
		{
			for (i in index...bpmChangeMap.length)
				if (bpmChangeMap[i].beatTime > beatTime)
					return i - 1;
			return bpmChangeMap.length - 1;
		}
	}
	...
	public static function getMeasuresInChangeIndex(measureTime:Float, index:Int = 0):Int
	{
		...
		{
			while (--index > 0)
				if (measureTime > bpmChangeMap[index].measureTime)
					return index;
			return 0;
		}
		else
		{
			for (i in index...bpmChangeMap.length)
				if (bpmChangeMap[i].measureTime > measureTime)
					return i - 1;
			return bpmChangeMap.length - 1;
		}
	}
	...
	public static function getStepsWithIndexInBPM(stepTime:Float, index:Int):Float
	{
		...
		{
			var prevBPM = bpmChangeMap[index - 1].bpm;
			if (stepTime <= bpmChange.stepTime)
				return prevBPM;

			var endStepTime = bpmChange.stepTime
				+ (bpmChange.endSongTime - bpmChange.songTime) * (bpmChange.bpm - prevBPM) / Math.log(bpmChange.bpm / prevBPM) / 15000;
			if (stepTime < endStepTime)
				return FlxMath.remapToRange(stepTime, bpmChange.stepTime, endStepTime, prevBPM, bpmChange.bpm);
		}
		return bpmChange.bpm;
	}
	...
	public static function getTimeWithBPMInSteps(time:Float, index:Int, bpm:Float):Float
	{
		...
		if (bpmChange.continuous && time > bpmChange.songTime && index > 0)
		{
			...
			if (time > bpmChange.endSongTime)
				return bpmChange.stepTime
					+ (((bpmChange.endSongTime - bpmChange.songTime) * (bpmChange.bpm - prevBPM)) / Math.log(bpmChange.bpm / prevBPM)
						+ (time - bpmChange.endSongTime) * bpm) / 15000;
			else
	...
	public static function getTimeInBeats(time:Float, from:Int = 0):Float
	{
		...
		if (index == -1)
			return time / (60000 / dummyChange.bpm);
		else if (index == 0)
			return time / (15000 / bpmChangeMap[index].bpm) / bpmChangeMap[index].stepsPerBeat;
		else
		{
	...
	public static function getStepsWithBPMInTime(stepTime:Float, index:Int, bpm:Float):Float
	{
		...
		if (bpmChange.continuous && stepTime > bpmChange.stepTime && index > 0)
		{
			...
			if (time > bpmChange.endSongTime)
				return (15000 * (stepTime - bpmChange.stepTime)
					- ((bpmChange.endSongTime - bpmChange.songTime) * (bpm - prevBPM)) / Math.log(bpm / prevBPM)) / bpm
					+ bpmChange.endSongTime;
			else
				return time;
		}
		else
	...
	public static function getMeasuresInTime(measureTime:Float, from:Int = 0):Float
	{
		...
		if (index == -1)
			return measureTime * (15000 / dummyChange.bpm);
		else if (index == 0)
			return measureTime * (15000 / bpmChangeMap[index].bpm) * bpmChangeMap[index].stepsPerBeat * bpmChangeMap[index].beatsPerMeasure;
		else
		{
			var change = bpmChangeMap[index];
	...
	public static function getBeatsInTime(beatTime:Float, from:Int = 0):Float
	{
		...
		if (index == -1)
			return beatTime * (15000 / dummyChange.bpm);
		else if (index == 0)
			return beatTime * (15000 / bpmChangeMap[index].bpm) * bpmChangeMap[index].stepsPerBeat;
		else
		{
			var change = bpmChangeMap[index];
