...
class MainState extends FlxState
{
	...
	public override function create()
	{
		...
		{
			if (FileSystem.exists(quick_modsPath + "." + ext))
				isZipMod = true;
			if (FileSystem.exists(quick_modsPath + "/cnemod." + ext))
				isCneMod = true;
			if (isZipMod && isCneMod)
				break;
		}
		...
		var addonPaths = [
			...
			((ModsFolder.currentModFolder != null && !isZipMod) ? quick_modsPath + "/addons/" : null)
		];
		...
		for (path in addonPaths)
		{
			...
			for (addon in FileSystem.readDirectory(path))
			{
				...
				{
					if (Flags.ALLOWED_ZIP_EXTENSIONS.contains(Path.extension(addon)))
				...
				if (addon.startsWith("[LOW]"))
					_lowPriorityAddons.insert(0, data);
				else if (addon.startsWith("[HIGH]"))
					_highPriorityAddons.insert(0, data);
				else
					_noPriorityAddons.insert(0, data);
			}
		}
		#end
		...
		{
			if (!(lib is ZipFolderLibrary))
				continue;
			if (cast(lib, ZipFolderLibrary).PRELOAD_VIDEOS)
