			...
			if (Flags.MOD_REDIRECT_STATES.exists(stateName))
			{
				@:privateAccess {
					...
					if (classFromString != null)
						FlxG.game._requestedState = Type.createInstance(classFromString, []);
					else
						FlxG.game._requestedState = new funkin.backend.scripting.ModState(Flags.MOD_REDIRECT_STATES.get(stateName));
				}
			}
		});
