...
class Week
{
	...
	public static function loadWeekCharacter(charName:String):WeekCharacter
	{
		...
		try
			...
			Logs.trace('Cannot parse character "$charName.xml": ${Std.string(e)}', ERROR);
		if (char == null)
			return null;

		if (!char.has.name)
			char.x.set("name", charName);
		if (!char.has.sprite)
			char.x.set("sprite", 'menus/storymenu/characters/${char.att.name}');
		if (!char.has.updateHitbox)
			char.x.set("updateHitbox", "true");
