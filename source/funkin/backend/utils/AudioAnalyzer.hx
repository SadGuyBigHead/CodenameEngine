...
final class WindowFunctions
{
	...
	public static inline function blackmanNuttall(x:Float):Float
		return 0.3635819
			- 0.4891775 * FlxMath.fastCos(TWO_PI * x)
			+ 0.1365995 * FlxMath.fastCos(FOUR_PI * x)
...
final class AudioAnalyzer
{
	...
	{
		if (wordSize == 2)
			return ArrayBufferIO.getInt16(buffer, position);
		else if (wordSize == 3)
		{
			wordSize = ArrayBufferIO.getUint16(buffer, position) | (buffer.get(position + 2) << 16);
			if (wordSize & 0x800000 != 0)
				return wordSize - 0x1000000;
			else
				return wordSize;
		}
		else if (wordSize == 4)
			return ArrayBufferIO.getInt32(buffer, position);
		else
			return ArrayBufferIO.getInt8(buffer, position);
	}
	 ...
	 */
	public static function getSpectrumFromFrequencies(frequencies:Array<Float>, sampleRate:Int, barCount:Int, ?spectrum:Array<Float>, ratio = 0.0,
			minDb = -63.0, maxDb = -10.0, minFreq = 20.0, maxFreq = 20000.0):Array<Float>
	{
		if (spectrum == null)
			spectrum = [];
		if (spectrum.length != barCount)
			spectrum.resize(barCount);

		var logMin = Math.log(minFreq), n = frequencies.length - 1;
		var logRange = Math.log(maxFreq) - logMin,
			dbRangeRate = 1 / (maxDb - minDb),
			rate = frequencies.length * 2 / sampleRate;
		...
		for (i in 0...barCount)
		{
			if ((range = (s2 = calculateScale(i + 1)) - s1) < 1)
			{
				i2 = Math.ceil(s2);
				if (i2 == i1)
					v = frequencies[i1] * range;
				else
					v = (frequencies[i1] + (frequencies[i2] - frequencies[i1]) * (s1 - i1)) * range;
			}
			else
			{
			...
			if (ratio > 0 && ratio < 1 && v < spectrum[i])
				spectrum[i] -= (spectrum[i] - v) * ratio;
			else
				spectrum[i] = v;
		}
	...
	public static function getLevelsFromFrequencies(frequencies:Array<Float>, sampleRate:Int, barCount:Int, ?levels:Array<Float>, ratio = 0.0, minDb = -63.0,
			maxDb = -10.0, minFreq = 20.0, maxFreq = 22000.0):Array<Float>
	...
	public static function getFrequenciesFromSamples(samples:Array<Float>, ?window:WindowFunction, ?frequencies:Array<Float>, ?fftN:Int):Array<Float>
	{
		...
		while ((fftN >>= 1) > 0)
			bits++;
		if (bits == 0)
			throw "FunkinAudioAnalyzer.getFrequenciesFromSamples: Cannot insert a sample length or fftN of 1";

		fftN = 1 << bits;
		var fftN2 = fftN >> 1, n = fftN - 1;

		var permutation:Array<Int>,
			twiddleReal:Array<Float>,
			twiddleImag:Array<Float>;
		_mutex.acquire();

		var real:Array<Float> = _reals[_freqCalculating],
			imag:Array<Float> = _imags[_freqCalculating];
		if (real == null)
		{
			_reals.push(real = []);
			_imags.push(imag = []);
		...
		if (frequencies == null)
			frequencies = [];
		if (frequencies.length != fftN2)
			frequencies.resize(fftN2);
		...
		for (i in 0...fftN)
		{
			...
			if (window != null)
				real[permutation[i]] *= window(i * tr);
			imag[i] = 0;
		}
		...
		for (i in 1...i0)
			frequencies[i] = 2 * Math.sqrt(real[i] * real[i] + imag[i] * imag[i]) * tr;
	 ...
	 * @return	Output of spectrum/bars that ranges from 0 to 1.
	 */
	public function getSpectrum(?pos:Float, ?timeUnit:TimeUnit, ?gain:Float, ?window:WindowFunction, barCount:Int, ?spectrum:Array<Float>, ?ratio:Float,
			?minDb:Float, ?maxDb:Float, ?minFreq:Float, ?maxFreq:Float):Array<Float>
	{
		return getSpectrumFromFrequencies(_frequencies = getFrequencies(pos, timeUnit, gain, window, _frequencies), data.sampleRate, barCount, spectrum,
			ratio, minDb, maxDb, minFreq, maxFreq);
	}
	...
	public function getLevels(?startPos:Float, ?volume:Float, barCount:Int, ?levels:Array<Float>, ?ratio:Float, ?minDb:Float, ?maxDb:Float, ?minFreq:Float,
			?maxFreq:Float):Array<Float>
	...
	public function getFrequencies(?pos:Float, ?timeUnit:TimeUnit, ?gain:Float, ?window:WindowFunction, ?frequencies:Array<Float>):Array<Float>
	{
		if (pos == null)
		{
			if (sound == null)
				return frequencies;
			_check();
			if ((pos = sound.time / 1000 * data.sampleRate - fftN) < 0)
				pos = 0;
			timeUnit = SAMPLE;
		}
	...
	public function analyze(startPos:Float, endPos:Float, ?timeUnit:TimeUnit, ?outOrOutMins:Array<Float>, ?outMaxs:Array<Float>):Float
	{
		...
		for (i in 0...data.channels)
			_mins[i] = _maxs[i] = -0x7FFFFFFF;
		if (startPos > endPos)
			_read(Math.floor(startPos * conversion), Math.floor(endPos * conversion), _analyzeRead);
		...
		for (i in 0...data.channels)
		{
			if (hasTwoOut)
			{
				if ((f = _mins[i] * v) > outOrOutMins[i])
					outOrOutMins[i] = f;
				if ((f = _maxs[i] * v) > outMaxs[i])
					outMaxs[i] = f;
			}
			if (_maxs[i] > max)
				max = _maxs[i];
		...
		{
			if ((f = min * v) > outOrOutMins[0])
				outOrOutMins[0] = f;
			if ((f = max * v) > outOrOutMins[1])
				outOrOutMins[1] = f;
		}
		return (max + min) * v;
	}

	function _analyzeRead(b:Int, c:Int)
		((b > _maxs[c]) ? (_maxs[c] = b) : (if (-b > _mins[c]) (_mins[c] = -b)));
	 ...
	 */
	public function getSamples(startPos:Float, ?timeUnit:TimeUnit, length:Int, mono = true, channel = -1, gain = 1.0, ?output:Array<Float>,
			?outputMerge = false):Array<Float>
	{
		_check();
		((!mono && channel == -1) ? (_sampleOutputLength = length * data.channels) : (_sampleOutputLength = length));
		if (((output == null) ? (_sampleOutput = output = []) : (_sampleOutput = output)).length != _sampleOutputLength)
			output.resize(_sampleOutputLength);
		...
		if (length > 0)
			_read(samplePos, samplePos + length, mono ? _getSamplesCallbackMono : (channel == -1 ? _getSamplesCallback : _getSamplesCallbackChannel));

		_sampleOutput = null;
		return output;
	}

	function _getSamplesCallbackMono(b:Int, c:Int):Void
		if (_sampleIndex < _sampleOutputLength)
		{
			if (c == 0)
				_sampleValue = idiv(b, data.channels);
			else
				_sampleValue += idiv(b, data.channels);
			if (c == _sampleChannel)
			{
				if (_sampleOutputMerge)
					_sampleOutput[_sampleIndex] += _sampleValue / byteSize;
				else
					_sampleOutput[_sampleIndex] = _sampleValue / byteSize;
				_sampleIndex++;
			}
	...
	function _getSamplesCallbackChannel(b:Int, c:Int):Void
		if (_sampleIndex < _sampleOutputLength)
		{
			if (c == _sampleChannel)
			{
				if (_sampleOutputMerge)
					_sampleOutput[_sampleIndex] += b / byteSize;
				else
					_sampleOutput[_sampleIndex] = b / byteSize;
				_sampleIndex++;
			}
	...
	function _getSamplesCallback(b:Int, c:Int):Void
		if (_sampleIndex < _sampleOutputLength)
		{
			if (_sampleOutputMerge)
				_sampleOutput[_sampleIndex] += b / byteSize;
	...
	public function read(startPos:Float, endPos:Float, ?timeUnit:TimeUnit, callback:ReadCallback)
	{
		...
		if (startPos > endPos)
			_read(Math.floor(startPos * conversion), Math.floor(endPos * conversion), callback);
	}
	...
	function _read(startSample:Int, endSample:Int, callback:ReadCallback)
	{
		...
		if (data.buffer.data != null)
			_readData(startSample * _sampleSize, endSample * _sampleSize, callback);
		...
		// if not use decoder and use seeking instead*
			...
			_readStream(startSample, endSample, callback);
	...
	{
		if (endIndex > data.buffer.data.byteLength)
			endIndex = data.buffer.data.byteLength;
