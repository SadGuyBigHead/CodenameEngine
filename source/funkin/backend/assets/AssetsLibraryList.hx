...
class AssetsLibraryList extends AssetLibrary
{
	...
	public function existsSpecific(id:String, type:String, source:AssetSource = BOTH)
	{
		...
		var cacheLibraryTypes = existsSpecificCacheLibrary.get(source),
			cacheTimeTypes = existsSpecificCacheTime.get(source);
		...
		var cacheLibraryPaths = cacheLibraryTypes.get(type),
			cacheTimePaths = cacheTimeTypes.get(type);
