...
class DialogueCutscene extends ScriptedCutscene
{
	...
	public inline function set_dialogueScript(val:Script)
		return script = val;

	public inline function get_dialogueScript()
		return script;
	...
	public override function create()
	{
		...
		try
		{
			...
			for (char in dialogueData.nodes.char)
			{
				...
				if (char.has.defaultAnim)
					leChar.defaultAnim = char.att.defaultAnim;
			...
			for (node in dialogueData.nodes.line)
			{
				...
				if (node.has.textSound)
					line.textSound = FlxG.sound.load(Paths.sound(node.att.textSound));
				else if (!useDef)
				{
	...
	public override function update(elapsed:Float)
	{
		...
		if (controls.ACCEPT)
		{
			if (dialogueBox.dialogueEnded)
				next();
			else
				dialogueBox.text.skip();
		}
	}
	...
	public function next(playFirst:Bool = false)
	{
		...
		{
			var force:Bool;
			if (force = (curLine.changeDefAnim != null))
				char.defaultAnim = curLine.changeDefAnim;
			dialogueBox.popupChar(char, force);
		}

		var finalSuffix:String = event.playFirst
			&& dialogueBox.hasAnimation(curLine.bubble + "-firstOpen") ? "-firstOpen" : dialogueBox.hasAnimation(curLine.bubble + "-open") ? "-open" : "";
		dialogueBox.playBubbleAnim(curLine.bubble, finalSuffix, curLine.text, curLine.format, curLine.speed, curLine.nextSound,
			curLine.textSound != null ? [curLine.textSound] : null, finalSuffix == "-firstOpen" || finalSuffix == "-open", !event.playFirst);

		if (curLine.playSound != null)
			curLine.playSound.play();
		...
		else if (curLine.musicVolume != null && curMusic != null)
			curMusic.volume = curLine.musicVolume;
	...
	public override function close()
	{
		...
		for (c in charMap)
			c.dialogueCharScript.call("close", [event]);
		if (dialogueBox != null)
	...
	{
		if (curMusic != null && !curMusic.persist)
			curMusic.destroy();
