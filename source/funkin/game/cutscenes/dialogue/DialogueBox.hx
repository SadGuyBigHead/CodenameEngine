...
class DialogueBox extends FunkinSprite
{
	...
	public function new(name:String)
	{
		...
		try
		{
			...
			if (!dialogueBoxData.has.sprite)
				dialogueBoxData.x.set("sprite", name);
			...
			if (dialogueBoxData.has.textSound)
				textTypeSFX = Paths.sound(dialogueBoxData.att.textSound);
			if (dialogueBoxData.has.nextSound)
				nextSFX = Paths.sound(dialogueBoxData.att.nextSound);
			...
			text = new FlxTypeText(textNode.has.x ? Std.parseFloat(textNode.att.x).getDefaultFloat(0) : 0,
	...
	public function popupChar(char:DialogueCharacter, force:Bool = false)
	{
		...
		if (event.cancelled || event.char == null || !active)
			return;
	...
	public function playBubbleAnim(bubble:String, suffix:String = '', text:String = '', ?format:Array<XMLUtil.TextFormat>, speed:Float = 0.05,
			?customSFX:FlxSound, ?customTypeSFX:Array<FlxSound>, setTextAfter:Bool = false, allowDefault:Bool = true)
	{
		var event = EventManager.get(DialogueBoxPlayBubbleEvent)
			.recycle(bubble, suffix, text, format, speed, customSFX, customTypeSFX, setTextAfter, allowDefault);
		dialogueBoxScript.call("playBubbleAnim", [event]);
		if (event.cancelled)
			return;

		if (event.customSFX != null)
			event.customSFX.play();
		else if (event.allowDefault)
			FlxG.sound.play(nextSFX);
		var idk:Void->Void = () ->
		{
			if (text != null && text.trim().length > 0)
				startText(event.text, event.format, event.speed, event.customTypeSFX);
			else
				dialogueEnded = true;
		}
	...
	{
		if (event == null)
			event = EventManager.get(DialogueBoxSetTextEvent).recycle(text, formats, null, null);
