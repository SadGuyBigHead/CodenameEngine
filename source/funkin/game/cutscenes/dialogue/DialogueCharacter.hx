...
class DialogueCharacter extends FunkinSprite
{
	...
	public function new(name:String, position:String)
	{
		...
		try
		{
			...
			if (!charData.has.sprite)
				charData.x.set("sprite", name);
			if (!charData.has.updateHitbox)
				charData.x.set("updateHitbox", "true");
			XMLUtil.loadSpriteFromXML(this, charData, defPath, LOOP);

			animation.finishCallback = (name:String) ->
			{
				if (finishAnimCallback != null)
					finishAnimCallback(name);

				if (name.endsWith("-show") || name == 'show')
					hasAnimation(defaultAnim) ? playAnim(defaultAnim, true) : animation.stop();
				else if (name.endsWith("-hide") || name == 'hide')
					alpha = 0;
			}
	...
	public function show(x:Float, y:Float, ?animation:String, force:Bool = false)
	{
		...
		if (event.cancelled || event.lastAnimContext == curAnimContext)
			return;
		...
		else
		{
			...
			alpha = 0;
			curTween = FlxTween.tween(this, {alpha: 1, y: event.y}, 0.2,
				{ease: FlxEase.quintOut, onComplete: function(twn:FlxTween) dialogueCharScript.call("showTweenCompleted", [twn])});
		}
	...
	public function hide(?animation:String, force:Bool = false)
	{
		...
		var anim:String;
		if (hasAnimation(anim = '${event.animation}-hide') || hasAnimation(anim = 'hide'))
			playAnim(anim, true);
		else
		{
			if (curTween != null)
				curTween.cancel();
			curTween = FlxTween.tween(this, {alpha: 0, y: y + 100}, 0.2,
				{ease: FlxEase.quintIn, onComplete: function(twn:FlxTween) dialogueCharScript.call("hideTweenCompleted", [twn])});
		}
