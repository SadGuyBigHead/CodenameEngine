...
class Cutscene extends MusicBeatSubstate
{
	...
	{
		if (!val)
			pauseItems.remove('Skip Cutscene');
		else if (!pauseItems.contains('Skip Cutscene'))
			pauseItems.insert(1, 'Skip Cutscene');
	...
	public function onSkipCutscene(event:NameEvent)
		close();

	public function onRestartCutscene(event:NameEvent)
		event.name = "Restart Song"; // Making the whole PlayState restart just for precaution  - Nex

	public function onResumeCutscene(event:NameEvent)
		event.name = "Resume";
	...
	public function selectPauseOption(event:NameEvent)
	{
		...
		{
			case 'Skip Cutscene':
				onSkipCutscene(event);
			case 'Restart Cutscene':
				onRestartCutscene(event);
			case 'Resume Cutscene':
				onResumeCutscene(event);
		}
