...
class Strum extends FlxSprite
{
	...
	**/
	...
	{
		return getPressed != null ? getPressed(strumLine) : strumLine.members.length != 4 ? ControlsUtil.getPressed(strumLine.controls,
			strumLine.members.length + "k" + ID) : switch (ID)
			{
	...
	{
		return getJustPressed != null ? getJustPressed(strumLine) : strumLine.members.length != 4 ? ControlsUtil.getJustPressed(strumLine.controls,
			strumLine.members.length + "k" + ID) : switch (ID)
			{
	...
	{
		return getJustReleased != null ? getJustReleased(strumLine) : strumLine.members.length != 4 ? ControlsUtil.getJustReleased(strumLine.controls,
			strumLine.members.length + "k" + ID) : switch (ID)
			{
	...
	**/
	public function updateNotePosition(daNote:Note)
	{
		...
		if (copyStrumScrollX)
			daNote.scrollFactor.x = scrollFactor.x;
		if (copyStrumScrollY)
			daNote.scrollFactor.y = scrollFactor.y;
