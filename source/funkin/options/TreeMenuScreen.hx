...
class TreeMenuScreen extends FlxSpriteGroup
{
	...
	**/
	...
	public inline function getID(name):String
		return prefix + name;

	public function translate(name:String, ?args:Array<Dynamic>):String
		return TU.translate(getID(name), args);
	...
	public function reloadStrings()
	{
		...
		for (object in members)
			if (object != null && object is OptionType)
				cast(object, OptionType).reloadStrings();
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (__firstFrame)
		{
			...
			if (members[curSelected] is ITreeOption)
			{
				...
				if (curOption is ITreeFloatOption)
					curFloatOption = cast curOption;
			}
			updateItems(true);
			return;
		}
		...
		if (inputEnabled)
		{
			...
			var change = (upTurboControl.activated ? -1 : 0) + (downTurboControl.activated ? 1 : 0) - FlxG.mouse.wheel,
				mouseControl = false;
			...
			{
				if (controls.ACCEPT || (!mouseControl && FlxG.mouse.justPressed && mouseOverlaps(members[curSelected])))
					curOption.select();
				...
				{
					if (leftTurboControl.activated)
						curOption.changeSelection(-1);
					if (rightTurboControl.activated)
						curOption.changeSelection(1);
				}
			}

			if (controls.BACK || (FlxG.mouse.justPressedRight && Main.timeSinceFocus > 0.3))
				close();
		}
