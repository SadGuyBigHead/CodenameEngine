...
class GithubIconOption extends TextOption
{
	public var user(default,
		null):CreditsGitHubContributor; // Can possibly be GitHubUser or GitHubContributor, but CreditsGitHubContributor has only the fields we need
	...
	{
		if (icon == null)
			return usePortrait = false;
		icon.shader = (value ? new CustomShader('engine/circleProfilePicture') : null);
		return usePortrait = value;
	}

	public function new(user:CreditsGitHubContributor, desc:String, ?callback:Void->Void, ?customName:String, size:Int = 96, usePortrait:Bool = true,
			waitUntilLoad:Float = 0.25)
	{
...
class GithubUserIcon extends FlxSprite
{
	...
	{ // Making the image download only if the player actually sees it on the screen  - Nex
		if (waitUntilLoad <= 0)
		{
			waitUntilLoad = null;
			Main.execAsync(function()
			{
				...
				if (bmap == null)
				{
					...
					{
						try
							bytes = HttpUtil.requestBytes('${user.avatar_url}?size=$size')
						catch (e)
							Logs.error('Failed to download github pfp for ${user.login}: ${CoolUtil.removeIP(e.message)} - (Retrying using the api..)');
					...
					{
						if (unfLink)
							user = cast GitHub.getUser(user.login,
								function(e)
									Logs.error('Failed to download github user info for ${user.login}: ${CoolUtil.removeIP(e.message)}')); // Api part - Nex
						try
							bytes = HttpUtil.requestBytes('${user.avatar_url}&size=$size')
						catch (e)
							Logs.error('Failed to download github pfp for ${user.login}: ${CoolUtil.removeIP(e.message)}');
