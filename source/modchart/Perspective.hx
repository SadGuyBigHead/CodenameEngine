...
final class Perspective
{
	...
	{
		this.origin = origin ?? new Vector3D(FlxG.width * .5, FlxG.height * .5, .0);
		fov = Math.PI / 2;
		updateProperties();
	}
	...
	//	vector.subtractToOutput(origin, vector);
	//	final projectedZ = __depthScale * Math.min((vector.z / FlxG.width) - 1, 0) + __depthOffset;
	//	final projectedFov = (__tanHalfFov / projectedZ);
	//	vector.setTo(vector.x * projectedFov, vector.y * projectedFov, projectedZ);
	//	vector.w = projectedZ;
	//	vector.addToOutput(origin, vector);
	//	return vector;
	// }
}
